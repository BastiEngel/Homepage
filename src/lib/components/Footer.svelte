<script lang="ts">
	...
	const platformIcons: Record<string, string> = {
		...
		Folkwang: 'M0 0h6v24H0V0zm9 0h15v8H9V0zm0 11h12v8H9v-8z',
	...
	<div class="mx-auto max-w-6xl text-center" use:scrollReveal>
		{#if config.meta.socialLinks.length > 0}
			<div class="mt-6 flex justify-center gap-4">
				{#each config.meta.socialLinks as link}
					<a
						href={link.url}
						target="_blank"
						rel="noopener noreferrer"
						class="text-text-muted hover:text-line flex h-10 w-10 items-center justify-center rounded-full transition-colors duration-200"
		...
		<p class="text-text-muted mt-8 text-sm">
			&copy; {currentYear} {config.meta.name}
		</p>

		<div class="mt-3 flex flex-col items-center gap-1">
			<a href="{base}/privacy" class="text-text-muted hover:text-line text-xs transition-colors duration-200">privacy policy</a>
			<a href="{base}/imprint" class="text-text-muted hover:text-line text-xs transition-colors duration-200">imprint</a>
		</div>

		<!-- SVG line endpoint -->
		<div class="mt-4 flex justify-center" aria-hidden="true">
			<svg width="12" height="12" viewBox="0 0 12 12">
				<circle cx="6" cy="6" r="5" fill="var(--color-line)" />
			</svg>
		</div>
	</div>
</footer>
